...
## 15. Interview Question & Answer Guide

Use these concise, authoritative answers during technical interviews:

### Q1: Why build a Fact Knowledge Layer instead of a standard RAG chatbot?
> **Answer**: Standard RAG flattens documents into semantic text chunks and queries an LLM to generate answers. This causes two major failure modes in enterprise and financial data:
> 1. LLMs hallucinate numbers and cannot perform exact unit normalization (e.g. ₹8,142 Cr vs ₹81,415 Mn).
> 2. LLMs naively flag differences across quarters or reporting scopes as "contradictions."
> A Fact Knowledge Layer extracts structured, normalized, and page-grounded facts *first*, and applies deterministic reconciliation rules *before* using generative models.

### Q2: How are facts represented?
> **Answer**: As structured Pydantic objects containing: `subject`, `predicate`, `raw_value`, `normalized_value`, `raw_unit`, `normalized_unit`, `period`, `scope`, `page_number`, `evidence_text`, and `confidence`. This guarantees that every fact has both canonical numeric grounding and verbatim auditability.

### Q3: How do you handle divergent units?
> **Answer**: Through deterministic unit scaling in `FactNormalizer`:
> - Currency: Multiplies Crore by 10 to reach canonical `INR million` base.
> - Tonnage: Converts `K tonnes` ($\times 1,000$) and `Mn tons` ($\times 1,000,000$) into `tonnes`.
> - Temporal: Normalizes variants like `FY 2023-24`, `Fiscal 2024`, and `FY24` into canonical `FY24`.

### Q4: How do you distinguish contradiction from contextual difference?
> **Answer**: We use a 7-step hierarchical decision tree:
> 1. If periods differ (e.g. `Q4 FY24` vs `FY24`, or `FY23` vs `FY24`), it is immediately classified as `CONTEXTUAL`.
> 2. If scopes differ (e.g. `since inception` vs `FY24 annual volume`), it is classified as `CONTEXTUAL`.
> 3. Only when subject, metric, period, and scope align, we compute relative numerical discrepancy $\delta = \frac{|v_a - v_b|}{\max(|v_a|, |v_b|)}$. If $\delta \le 1.5\%$, it is `CORROBORATED`. If $\delta > 1.5\%$, it is a genuine `CONTRADICTION`.

### Q5: Why not use an LLM for everything?
> **Answer**: LLMs are non-deterministic, expensive, slow, and prone to mathematical hallucinations. For tasks like unit conversion (Cr $\to$ Mn) and temporal checks (Q4 $\neq$ FY), deterministic code is $1000\times$ faster, 100% reproducible, and zero-cost. We reserve LLM reasoning solely for nuanced semantic ambiguity.

### Q6: Why SQLite, FastAPI, and Streamlit?
> **Answer**:
> - **SQLite**: Zero-infrastructure, embedded ACID database with WAL mode and foreign key enforcement, perfect for reliable single-node or containerized workflows.
> - **FastAPI**: Modern, asynchronous, typed REST API with automatic OpenAPI documentation.
> - **Streamlit**: Python-native interactive UI that enables rapid exploration of tabular data, evidence snippets, and visual reconciliation cards without frontend build overhead.

---

## 16. Limitations & Future Work

- **Complex Nested Tables**: Currently, PyMuPDF extracts text streams and heuristics parse tabular rows. Adding table structure decoders (e.g., `pdfplumber` table line-art recognition) will improve multi-column dense balance sheets.
- **Multilingual Reports**: Expansion to regional language annual reports via OCR and multilingual embeddings.
- **Automated Anomaly Alerts**: Pushing contradiction alerts directly to communication webhooks (Slack / Teams / Email).

---

## 17. How to Demonstrate the Project in Under 3 Minutes

1. **Terminal 1**: Run `python scripts/evaluate_demo_cases.py` to show all 4 benchmark cases pass with exact mathematical explanations.
2. **Terminal 2**: Run `python run.py frontend` and open `http://localhost:8501`.
3. In the UI:
   - Click **Dashboard** to show the metric cards and distribution of relationships.
   - Click **Fact Comparison** and filter by `CORROBORATED` to show the real Delhivery ₹81,415 Mn vs ₹8,142 Cr equivalence.
   - Filter by `CONTEXTUAL` to show Q4 vs Full Year reporting.
   - Click **Evidence Explorer** to show the exact source PDF page and verbatim quote.
